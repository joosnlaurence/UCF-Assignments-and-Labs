...
int enqueue(struct queue* qPtr, int val) {
 ...
 //see the slide for visualization
    if(qPtr->numElements >= qPtr->queueSize){ //realloc the queue to double the size
        size_t newSize = sizeof(int)*qPtr->queueSize*2;
        qPtr->elements = (int*)realloc(qPtr->elements, newSize);
        for(int i=0; i<qPtr->front; i++){
            int enqIndex = i + qPtr->numElements;
            qPtr->elements[enqIndex] = qPtr->elements[i];
        }
    }
    int enqIndex = (qPtr->front + qPtr->numElements) % qPtr->queueSize;
    qPtr->numElements++;
    qPtr->elements[enqIndex] = val;
...
– return val*/
    ...
    int n = qPtr->elements[qPtr->front];
